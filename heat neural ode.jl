...
# ╔═╡ 359eab01-5f59-474f-ae71-ebc499c67f81
using ComponentArrays, Lux, DiffEqFlux, OrdinaryDiffEq, Optimization, OptimizationOptimJL,
      OptimizationOptimisers, Random, Plots

# ╔═╡ dfd91983-8f39-4f04-8fe0-29b95e7dd3b5
md"""
# Neural ODE
https://docs.sciml.ai/DiffEqFlux/stable/examples/neural_ode/
"""

# ╔═╡ 67cb037c-a9a3-42a7-8438-d3e0f2d8b8d4
rng = Xoshiro(0)

# ╔═╡ be88c1d4-ed0d-4265-a500-dfc04586b30d
N = 3

# ╔═╡ 38f24b01-7572-4bae-8f6e-e5db82fc9a46
u0 = collect(rand(Float32, N,N))

# ╔═╡ f8d51ea0-86df-4f8a-a655-59fc0cc5532c
datasize = 30

# ╔═╡ c487743c-9733-4488-862e-960be36a088f
tspan = (0.0f0, 1.5f0)

# ╔═╡ cdc9e974-c364-4414-80ca-00aa25efe27b
tsteps = range(tspan[1], tspan[2]; length = datasize)

# ╔═╡ 8b0735af-cf85-4224-917e-9f707986420a
function trueODEfunc(du, u, p, t)
    Nx, Ny = size(u)
    # α, dx, dy = p  # thermal diffusivity and grid spacing
	α, dx, dy = ones(3)
    # second derivatives
    ...
    # Neumann BCs (zero flux = copy neighbor)
    du[1,:]    .= du[2,:]
    du[end,:]  .= du[end-1,:]
    du[:,1]    .= du[:,2]
    du[:,end]  .= du[:,end-1]
...
# ╔═╡ 169d5d2a-73fa-4560-81b5-4eaccbe9e1f4
prob_trueode = ODEProblem(trueODEfunc, u0, tspan)

# ╔═╡ 3288b0ca-63eb-4765-935c-486d15f84ed1
odesol = solve(prob_trueode, Tsit5(); saveat = tsteps)

# ╔═╡ 16026190-d37a-4f87-ae44-c9c50371e51c
scatter(odesol)

# ╔═╡ 466f9c6f-a7d9-4b12-858c-42f65b742d8a
ode_data = Array(odesol)

# ╔═╡ cff7eb01-c19d-47e3-8e9c-5a50840bea8a
heatmap(ode_data[:,:,end])

# ╔═╡ 85bc383a-dacd-498f-bb6e-575fcd8cda9a
dudt2 = Chain(x-> reshape(x,N*N),Dense(N*N, 2*N*N, tanh), Dense(2*N*N, N*N), x-> reshape(x,N,N))

# ╔═╡ ab373412-b4c3-4ee8-afd7-bb91bcbeeaa9
p, st = Lux.setup(rng, dudt2)

# ╔═╡ 65dd64fa-a840-4cc2-93fa-ebb34a78ec21
prob_neuralode = NeuralODE(dudt2, tspan, Tsit5(); saveat = tsteps)

# ╔═╡ 4f2e66a5-8417-40f7-ac29-beaf95862170
function predict_neuralode(p)
    Array(prob_neuralode(u0, p, st)[1])
end

# ╔═╡ 8bd908e3-9e59-4682-a9e2-a088bef5e965
...
# ╔═╡ 986f2bdb-de97-42c9-8f67-70c1bc294ee9
...
# ╔═╡ 74a78cc5-ff44-4005-8f5d-5f02c1d419b7
pinit = ComponentArray(p)

# ╔═╡ b8fbc34f-8579-48bc-9271-2888344cc758
begin
...
# ╔═╡ f24a4108-f61e-48e0-818b-ce0242b1488b
adtype = Optimization.AutoZygote()

# ╔═╡ 40499877-b1cd-4d7c-82c4-24e7a1e65123
optf = Optimization.OptimizationFunction((x, p) -> loss_neuralode(x), adtype)

# ╔═╡ e4c4ec8c-62e1-4bdc-b8b6-cca17ab14102
optprob = Optimization.OptimizationProblem(optf, pinit)

# ╔═╡ 4eb96bf5-8d7e-4e18-80aa-b63e28a4ea35
result_neuralode = Optimization.solve(
    optprob, OptimizationOptimisers.Adam(0.05); callback = callback, maxiters = 300)

# ╔═╡ 127888d6-2554-4875-bd5e-51683b89e935
optprob2 = remake(optprob; u0 = result_neuralode.u)

# ╔═╡ 1269cdf4-cb4b-4e31-8cc7-729e8be2d422
result_neuralode2 = Optimization.solve(
    optprob2, Optim.BFGS(; initial_stepnorm = 0.01); callback, allow_f_increases = false)

# ╔═╡ 527f633d-9281-4915-9f6c-31754f2f49f3
result_neuralode2.u.layer_2.weight

# ╔═╡ 6a6e2b9b-1f67-4aab-9918-8f5614162f21
begin
...
manifest_format = "2.0"
project_hash = "37d683c02a1d7f3d3f9902f3b172633da6a80a93"
...
[[deps.Boltz]]
deps = ["ADTypes", "ArgCheck", "Artifacts", "ChainRulesCore", "Compat", "ConcreteStructs", "Downloads", "ForwardDiff", "Functors", "LazyArtifacts", "Lux", "LuxCore", "MLDataDevices", "Markdown", "NNlib", "Random", "Reexport", "Scratch", "Setfield", "Static", "Statistics", "WeightInitializers"]
git-tree-sha1 = "85eba7d441a091c2e9a3c5417c3b652fc65b0639"
uuid = "4544d5e4-abc5-4dea-817f-29e4c205d9c8"
version = "1.6.0"

    [deps.Boltz.extensions]
    BoltzDataInterpolationsExt = "DataInterpolations"
    BoltzDynamicExpressionsExt = "DynamicExpressions"
    BoltzEnzymeExt = "Enzyme"
    BoltzJLD2Ext = "JLD2"
    BoltzMetalheadExt = "Metalhead"
    BoltzPickleExt = "Pickle"
    BoltzReactantExt = ["Reactant", "ReactantCore"]
    BoltzReverseDiffExt = "ReverseDiff"
    BoltzSafeTensorsExt = "SafeTensors"
...
[[deps.DiffEqFlux]]
deps = ["ADTypes", "Boltz", "ChainRulesCore", "ConcreteStructs", "Distributions", "LinearAlgebra", "Lux", "LuxCore", "LuxLib", "Random", "Reexport", "SciMLBase", "SciMLSensitivity", "Setfield", "Static"]
git-tree-sha1 = "163c3f6f0a4a592eb312fb927b59f883c1edb1cf"
uuid = "aae7a2af-3d4f-5e19-a356-7da93b79d9d0"
version = "4.4.0"

    [deps.DiffEqFlux.extensions]
    DiffEqFluxDataInterpolationsExt = "DataInterpolations"

    [deps.DiffEqFlux.weakdeps]
    DataInterpolations = "82cc6244-b520-54b8-b5a6-8a565e85f1d0"
...
git-tree-sha1 = "c30c4f0f9c3e769170fe938320097909561dda9c"
...
git-tree-sha1 = "b4168be5b9f7fe3448abc307300202e7c93261df"
...
git-tree-sha1 = "5e478a47ee745177e5a83072c6be9665ceecc7ef"
...
git-tree-sha1 = "e87bff9d8e63f458a51f4381d208e7ea6c3b4880"
...
git-tree-sha1 = "ba613278791453cf7d4dce7cd61c6786b96140cf"
...
git-tree-sha1 = "70d80b89ecefeabb7a1a812a03a3275a0db7f866"
...
git-tree-sha1 = "ee3794a2ca3022bc066b5fb87b135c1d5f3b1cb7"
...
[[deps.LuxLib]]
deps = ["ArrayInterface", "CPUSummary", "ChainRulesCore", "DispatchDoctor", "EnzymeCore", "FastClosures", "ForwardDiff", "Functors", "KernelAbstractions", "LinearAlgebra", "LuxCore", "MLDataDevices", "Markdown", "NNlib", "Preferences", "Random", "Reexport", "SciMLPublic", "Static", "StaticArraysCore", "Statistics"]
git-tree-sha1 = "2f80ab5ff577e247b5dc2df1a726e11aff5e78a0"
...
git-tree-sha1 = "3da3daaceafec5e49cac1920124e8d7430e79d83"
...
git-tree-sha1 = "ea6169605fe93e02df87874f388279ae766175d9"
...
git-tree-sha1 = "3daf56b14764a18b47ccd0b3560dc583a180727f"
...
[[deps.StructUtils]]
deps = ["Dates", "UUIDs"]
git-tree-sha1 = "40807d003c0eb88b10558d19b762d175332c93a4"
...
git-tree-sha1 = "c0a2569d1e88648616450ad065d1a5a0138a6d02"
